...
class SkeletonCLR(nn.Module):
    ...
    def forward(self, im_q, im_k=None):
        ...
        print(l_neg.shape)
        l_bopos = l_pos + torch.zeros(l_neg.shape[0], l_neg.shape[1]).cuda()

        # logits: Nx(1+K)
        logits = torch.cat([l_pos, l_neg], dim=1)

        print(l_bopos.shape)
        representations = torch.cat([l_pos, l_neg], dim=1)
        representations = representations.transpose(0,1)


        kmeans = KMeans(n_clusters=60, mode='cosine', verbose=0)
        labels = kmeans.fit_predict(representations)


        mask1 = torch.zeros(logits.shape[1])
        mask1 = mask1.bool().cuda()
        mask1[0] = True
        mask1[-1] = True
        num = 0
        print('yyyy',len(labels),logits.shape[1])
        print(labels[0],labels)

        mask1[labels==labels[0]]=False
        print(mask1)
        # print('yyy')
        # for j in range(1,logits.shape[1]-1):
